// components/PresencePermissionCard.tsx
...
const PresencePermissionCard = ({ nama, keterangan }: PresenceCardProps) => {
  const isMasuk = keterangan !== 'Absen Masuk';
  const cardColor = 'bg-yellow-50'; // Contoh warna berbeda

  return (
    <div className={`p-6 rounded-2xl shadow-md border-l-4 ${'border-yellow-500'} transition-all duration-300 transform hover:scale-105 ${cardColor}`}>
      ...
      <p className={`font-bold text-sm ml-8 -mt-3 ${'text-yellow-600'}`}>{keterangan}</p>
    </div>
  );
};

export default PresencePermissionCard;