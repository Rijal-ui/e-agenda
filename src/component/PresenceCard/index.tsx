  ...
  return (
    <div className={`p-6 rounded-2xl shadow-md border-l-4 ${isMasuk ? 'border-green-500' : 'border-yellow-500'} transition-all duration-300 transform hover:scale-105 ${cardColor}`}>
      <div className="flex items-center space-x-3 mb-2">
        <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 ${isMasuk ? 'text-green-500' : 'text-yellow-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
        </svg>
        <p className="font-bold text-gray-800 text-lg">Nama:</p>
      </div>
      <p className="text-blue-600 font-bold ml-9 -mt-3">{nama}</p>
      
      <div className="flex items-center space-x-3 mt-1 mb-2">
        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-gray-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
        <p className="font-bold text-gray-800 text-lg">Jam:</p>
      </div>
      <p className="text-gray-700 ml-9 -mt-3">{jam}</p>
      ...
      <div className="flex items-center space-x-3 mt-1 mb-2">
        ...
        <p className="font-bold text-gray-800 text-lg">Keterangan:</p>
      </div>
      <p className={`font-bold ml-9 -mt-3 ${isMasuk ? 'text-green-600' : 'text-yellow-600'}`}>{keterangan}</p>
    </div>
  );
};
